...
// internally nnil is the new null
...
// some haphazzard stuff the guy coded oof

void show_tree(struct node *root, struct trunk *prev, int is_left)
{
	if (root == nnil) return;

	struct trunk this_display = { prev, "  " };
	char *prev_str = this_display.str;
	show_tree(root->kid[0], &this_display, 1);
	...
	show_trunks(&this_display);
	printf("%d\n", root->payload);

	if (prev) prev->str = prev_str;
	this_display.str = "   |";

	show_tree(root->kid[1], &this_display, 0);
