...
/*
	no need to lock the object: at worst the readout would be 1 tick off, which is no worse than integrator's inate inaccuracy 
	d
*/
...
void update(integ x)
{
	...
	t = ((tv.tv_sec - x->start.tv_usec) * 1000000 + tv.tv_usec - x->start.tv_usec) * 1e-6;
...
void* tick(void *a)
{
	integ x = a;
	while (1) {
		usleep(1000000); // update every .1 sec 
		update(x);
	}
}
...
// gcc -Wall -o active_object active_object.c
// 